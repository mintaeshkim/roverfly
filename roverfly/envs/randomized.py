"""Randomized quadrotor-payload tracking environment."""
...
from roverfly.math.rotations import euler2rot, quat2euler_raw, quat2rot
...
class QuadrotorRandomEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        xml_file = resolve_model_path(xml_file, default="quadrotor_x_cfg_payload_nominal.xml")
    ...
    def _randomize_inertia(self):
        ...
        self.model.body_inertia[self.body_id] = (
            np.array([0.49, 0.53, 0.98]) * 1e-2 * np.clip(np.random.normal(1, 0.1, 3), -0.9, 1.1)
        )
    ...
    def _get_obs(self):
        ...
        ff = np.concatenate([(xQ_ff @ self.RQ).flatten(), (vQ_ff @ self.RQ).flatten()])  # 18

        return np.concatenate([self.obs_curr, io_history, ff]).astype(np.float32, copy=False)
    ...
    def _get_obs_curr(self):
        ...
        obs_curr = np.concatenate(
            [
                self.e_curr,
                self.RQ.T @ (self.xQ - self.xP),
                self.RQ.T @ (self.vQ - self.vP),
                self.RQ.T @ (self.xQ - self.xN1),
                self.RQ.T @ (self.vQ - self.vN1),
                self.RQ.T @ (self.xQ - self.xN2),
                self.RQ.T @ (self.vQ - self.vN2),
                self.action,
                self.RQ.flatten(),
                self.ωQ,
            ]
    ...
    def _render_marker(self):
        ...
        self.mujoco_renderer.viewer.add_marker(
            pos=marker_pos, size=np.array([0.05, 0.05, 0.05]), rgba=np.array([1, 0, 0, 1])
        )
    ...
    def _get_reward(self):
        ...
        rewards = np.exp(
            -np.array([scale_xQ, scale_vQ, scale_ψQ, scale_ωQ]) * np.array([exQ, evQ, eψQ, eωQ])
        )
    ...
    def _terminated(self):
        ...
        if exQ > self.pos_err_bound:
            ...
            logger.info(
                "Env {env_num} | Ep {epi} | St {stage} | Traj: {traj_type} | Pos error: {pos_err} | Time: {time} | Reward: {rew}".format(
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | St {stage} | Traj: {traj_type} | Vel error: {vel_err} | Time: {time} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    stage=self.stage,
                    traj_type=self.traj_type + " " + str(np.round(self.difficulty, 2)),
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | St {stage} | Traj: {traj_type} | Att error: {att} | Time: {time} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    stage=self.stage,
                    traj_type=self.traj_type + " " + str(np.round(self.difficulty, 2)),
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | St {stage} | Traj: {traj_type} | Max time: {time} | Final pos error: {pos_err} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    stage=self.stage,
                    traj_type=self.traj_type + " " + str(np.round(self.difficulty, 2)),
