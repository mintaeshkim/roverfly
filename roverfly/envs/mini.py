...
from numpy import abs, clip, concatenate, copy, exp, mean, pi, round, sqrt, sum, tanh
...
from roverfly.math.rotations import euler2quat_raw, euler2rot, quat2euler_raw
from roverfly.math.transforms import dual_tanh_quadrotor
...
class QuadrotorMiniEnv(BaseQuadrotorEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        xml_file = resolve_model_path(xml_file, default="quadrotor_mini.xml")
        ...
        self.future_len = 5
        self.s_buffer = deque(
            np.zeros((self.history_len, self.s_dim)), maxlen=self.history_len
        )  # [x, R, v, ω]
        self.d_buffer = deque(
            np.zeros((self.history_len, 6)), maxlen=self.history_len
        )  # [xQd, vQd]
        ...
        self.mP = 0.025  # [kg]
        self.g = 9.81
        self.JQ = np.array([[236.40, 0, 0], [0, 236.40, 0], [0, 0, 303.16]]) * 1e-6
    ...
    def _set_action_space(self):
        ...
        if self.is_action_bound:
            self.action_space = Box(low=0.2 * np.ones(4), high=0.8 * np.ones(4))
        else:
    ...
    def _reset_model(self):
        if not self.is_full_traj:
            self.max_timesteps = self.traj_timesteps
    ...
    def _generate_trajectory(self):
        ...
        if self.is_full_traj:
            ...
            self.x_offset = np.array(
                [
                    3 * uniform(low=-1, high=1),
                    3 * uniform(low=-1, high=1),
                    6 * uniform(low=0, high=1),
                ]
            )
    ...
    def _set_initial_state(self):
        ...
        attQ = euler2quat_raw(
            quat2euler_raw(self.init_qpos[3:7]) + uniform(size=3, low=-watt, high=watt)
        )
    ...
    def step(self, action, restore=False):
        ...
        if self.is_action_filter:
            self.action = self.action_filter.filter(action)
        if self.is_delayed:
    ...
    def do_simulation(self, ctrl, n_frames):
        ...
        if self.control_scheme == "ctbr":
            ctrl = self._ctbr2srt(ctrl)  # CTBR
        else:
            ctrl = self.rotor_max_thrust * (ctrl + 1) / 2  # SRT
    ...
    def _ctbr2srt(self, thrust_body_rates):
        ...
        ang_accel_d = np.array(
            [
                e_ang_vel[0] / self.angVelControl_timeConst_xy,
    ...
    def _terminated(self):
        ...
        if 5.0 <= self.time_in_sec <= 35.0 and xQ[2] < 0.01:  # Crash except for takeoff and landing
            ...
            logger.info(
                "Env {env_num} | Ep {epi} | Traj: {traj_type} | Crashed | Pos error: {pos_err} | Time: {time} | Reward: {rew}".format(
        ...
        if self.time_in_sec <= 35.0 and exQ > self.pos_err_bound:
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | Traj: {traj_type} | Pos error: {pos_err} | Time: {time} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    traj_type=self.traj_type + " " + str(round(self.progress[self.traj_type], 2)),
                    pos_err=round(exQ, 2),
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | Traj: {traj_type} | Vel error:  {vel_err} | Time: {time} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    traj_type=self.traj_type + " " + str(round(self.progress[self.traj_type], 2)),
                    vel_err=round(evQ, 2),
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | Traj: {traj_type} | Att error: {att} | Time: {time} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    traj_type=self.traj_type + " " + str(round(self.progress[self.traj_type], 2)),
                    att=round(attQ, 2),
            ...
            self.history_epi[self.traj_type].append(self.timestep)
            logger.info(
                "Env {env_num} | Ep {epi} | Traj: {traj_type} | Max time: {time} | Final pos error: {pos_err} | Reward: {rew}".format(
                    env_num=self.env_num,
                    epi=self.num_episode,
                    traj_type=self.traj_type + " " + str(round(self.progress[self.traj_type], 2)),
                    time=round(self.time_in_sec, 2),
