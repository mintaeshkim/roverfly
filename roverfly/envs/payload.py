...
from roverfly.control.filters import ActionFilterButter
from roverfly.envs.base import BaseQuadrotorEnv
from roverfly.envs.plotting import PayloadPlottingMixin
from roverfly.math.rotations import euler2quat_raw, euler2rot, quat2euler_raw, rot2euler
from roverfly.math.transforms import dual_tanh_srt, dual_tanh_tvec
from roverfly.paths import resolve_model_path
from roverfly.simulation.environment import MujocoEnv
from roverfly.simulation.randomization import EnvRandomizer

DEFAULT_CAMERA_CONFIG = {"trackbodyid": 0, "distance": 15.0}
logger = logging.getLogger(__name__)


class QuadrotorPayloadEnv(PayloadPlottingMixin, BaseQuadrotorEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        xml_file = resolve_model_path(xml_file, default="quadrotor_falcon_payload.xml")
        ...
        self.sim_freq: float = (
            500.0 if self.control_scheme in ["ctbr", "tvec"] else self.policy_freq
        )
        ...
        self.action_offset = (
            zeros(self.a_dim) if self.control_scheme in ["ctbr", "tvec"] else -0.4 * ones(4)
        )
        ...
        self.g = 9.81
        self.JQ = (
            np.array([[0.401, 0.0004, 0.0015], [0.0004, 0.358, 0.0004], [0.0015, 0.0004, 0.636]])
            * 1e-2
        )
        self.l = 0.1524
        ...
        [
            self.action_queue.append([self.data.time, self.action_last])
            for _ in range(self.action_queue_len)
        ]
    ...
    def _init_history_ff(self):
        ...
        d_curr = concatenate(
            [
                self.xQd[0] / self.pos_bound,
                self.xPd[0] / self.pos_bound,
                self.vQd[0] / self.vel_bound,
                self.vPd[0] / self.vel_bound,
            ]
        )
        ...
        if self.is_record_action:
            self.action_record = zeros((self.max_timesteps, self.a_dim))
        if self.is_record_xP:
            self.xP_record = zeros((self.max_timesteps, 3))

    def reset(self, seed=None, randomize=None):
        # super().reset(seed=self.env_num)
        if self.is_env_randomized:
            self.model, self.mP, self.cable_length, self.tau_up, self.tau_down = (
                self.env_randomizer.randomize_env(self.model)
            )
        self._reset_env()
        self._reset_model()
        self._reset_error()
    ...
    def _reset_model(self):
        if not self.is_full_traj:
            self.max_timesteps = self.track_timesteps
        ...
        if self.is_full_traj:
            self.traj = ut.FullCrazyTrajectoryPayload(tf=40, traj=self.traj)
    ...
    def _generate_trajectory(self):
        ...
        for i in range(self.max_timesteps + self.history_len):
            (
                ...
                self.dqd[i],
                self.d2qd[i],
            ) = self.traj.get(i * self.policy_dt)
    ...
    def _set_initial_state(self):
        ...
        wx = np.array([0, 0, self.cable_length])
        watt = np.zeros(3)
        wv = np.zeros(3)
        wω = np.zeros(3)
    ...
    def _get_obs(self):
        ...
        xQd = (
            self.xPd[self.timestep : self.timestep + self.future_len]
            - self.qd[self.timestep : self.timestep + self.future_len]
        )
        vQd = (
            self.vPd[self.timestep : self.timestep + self.future_len]
            - self.dqd[self.timestep : self.timestep + self.future_len]
        )
        xQ_ff = self.xQ - xQd
        vQ_ff = self.vQ - vQd
        ff = concatenate(
            [
                (xP_ff @ self.RQ).flatten(),
                (vP_ff @ self.RQ).flatten(),
                (xQ_ff @ self.RQ).flatten(),
                (vQ_ff @ self.RQ).flatten(),
            ]
        )  # 60

        return concatenate([self.obs_curr, io_history, ff]).astype(np.float32, copy=False)  # 306
    ...
    def _get_obs_curr(self):
        ...
        self.e_curr = concatenate(
            [self.RQ.T @ self.exP, self.RQ.T @ self.evP, self.RQ.T @ self.exQ, self.RQ.T @ self.evQ]
        )
    ...
    def _get_state_curr(self):
        ...
        self.ωQ = self.data.qvel[3:6] + clip(
            normal(loc=0, scale=pi / 30, size=3), -pi / 60, pi / 60
        )
    ...
    def step(self, action, restore=False):
        ...
        self.action = (
            self.action_filter.filter(self.raw_action) if self.is_action_filter else self.raw_action
        )
        if self.is_ema_action:
            self.action = 0.2 * self.action + 0.8 * self.action_last
        if self.is_delayed:
            self.action_queue.append([self.data.time, self.action])
        if self.is_full_traj:
            self._apply_downwash()
        if self.is_disturbance:
    ...
    def _step_mujoco_simulation(self, ctrl, n_frames):
        if self.is_rotor_dynamics:
            self.actual_forces = self._rotor_dynamics(ctrl)
        else:
            self.actual_forces = ctrl
    ...
    def _tvec2srt(self, action):
        ...
        thrust_vec = concatenate(
            [5 * tanh(action[:2]), [dual_tanh_tvec(action[2])]]
        )  # [Fx, Fy, Fz] (N)
        yaw_sp = 0
        ...
        e3_cmd = thrust_vec
        if np.isnan(norm(e3_cmd)) or norm(e3_cmd) < 1e-4:
            e3_cmd = e3
        ...
        self.rate_integral = clip(
            self.rate_integral + e_rate * self.sim_dt,
            -self.max_rate_integral,
            self.max_rate_integral,
        )
        rate_deriv = (
            e_rate - np.array([self.edφP_prev, self.edθP_prev, self.edψP_prev])
        ) / self.sim_dt
        ...
        M = (
            self.kp_rate * e_rate  # P term on rates
    ...
    def _update_data(self, reward, step=True):
        ...
        self.d_buffer.append(
            concatenate(
                [
                    self.xQd[self.timestep] / self.pos_bound,
                    self.xPd[self.timestep] / self.pos_bound,
                    self.vQd[self.timestep] / self.vel_bound,
                    self.vPd[self.timestep] / self.vel_bound,
                ]
            )
        )
        ...
        # Recording
        if self.is_record_action and self.timestep < self.max_timesteps:
            self.action_record[self.timestep] = self.action
        if self.is_record_xP and self.timestep < self.max_timesteps:
            self.xP_record[self.timestep] = self.data.qpos[7:10]
    ...
    def _get_reward(self):
        ...
        errors = {
            ...
            "Δa": sum(
                exp(-np.linspace(0, 1, self.history_len))
                * [
                    norm(a - b)
                    for a, b in zip(
                        list(self.a_buffer)[1:] + [self.action], list(self.a_buffer) + [self.action]
                    )
                ]
            )
            / sum(exp(-np.linspace(0, 1, self.history_len))),
            "dq": norm(
                (self.data.qvel[0:3] - self.data.qvel[6:9]) / self.cable_length
                if not self.cable_length == 0
                else 0
            ),
        }
        ...
        total_reward = (
            (weighted_rewards["xP"] + weighted_rewards["ψQ"])
            * (1 + weighted_rewards["ωQ"] + weighted_rewards["dq"])
            + weighted_rewards["a"]
            + weighted_rewards["Δa"]
        )
    ...
    def _terminated(self):
        ...
        if 5.0 <= self.time_in_sec <= 35.0 and xQ[2] < 0:  # Crash except for takeoff and landing
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Crashed | Time: {round(self.time_in_sec, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            if self.is_record_action:
                self.plot_action()
            return True
        if exP > self.pos_err_bound:
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Pos error: {round(exP, 2)} | Time: {round(self.time_in_sec, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            if self.is_record_action:
                self.plot_action()
            return True
            ...
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Vel error: {round(evP, 2)} | Time: {round(self.time_in_sec, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            if self.is_record_action:
                self.plot_action()
            return True
            ...
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Att error: {round(attQ, 2)} | Time: {round(self.time_in_sec, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            if self.is_record_action:
                self.plot_action()
            return True
            ...
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Max time: {round(self.time_in_sec, 2)} | Final pos error: {round(exP, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            if self.is_record_xP:
                self.plot_xP()
                self.plot_xP_3d()
