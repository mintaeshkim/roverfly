...
from roverfly.control.filters import ContinuousActionFilter
...
from roverfly.math.transforms import dual_tanh_quadrotor, dual_tanh_tvec
...
class QuadrotorEnv(BaseQuadrotorEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        xml_file = resolve_model_path(xml_file, default="quadrotor_falcon.xml")
        ...
        self.future_len = 5
        self.s_buffer = deque(
            zeros((self.history_len, self.s_dim)), maxlen=self.history_len
        )  # [x, R, v, ω]
        ...
        self.action_offset = (
            zeros(self.a_dim)
            if self.control_scheme in ["ctbr", "tvec"]
            else -0.46 * ones(self.a_dim)
        )
        ...
        self.g = 9.81
        self.JQ = (
            np.array([[0.49, 0.00055, 0.002], [0.00055, 0.53, 0.00054], [0.002, 0.00054, 0.98]])
            * 1e-2
        )
        self.l = 0.1524
    ...
    def _reset_model(self):
        ...
        if self.is_full_traj:
            self.traj = ut.FullCrazyTrajectory(tf=40, traj=self.traj)
    ...
    def _generate_trajectory(self):
        ...
        self.x_offset = self.pos_bound * np.array(
            [uniform(-1, 1), uniform(-1, 1), 0 if self.is_full_traj else 2 * uniform(0.5, 1)]
        )
    ...
    def _get_obs(self):
        ...
        return concatenate([self.obs_curr, io_history, ff]).astype(np.float32, copy=False)
    ...
    def _get_state_curr(self):
        ...
        return concatenate(
            [self.xQ / self.pos_bound, self.R.flatten(), self.vQ / self.vel_bound, self.ω]
        )
    ...
    def _update_data(self, reward, step=False):
        ...
        self.d_buffer.append(
            concatenate(
                [self.xQd[self.timestep] / self.pos_bound, self.vQd[self.timestep] / self.vel_bound]
            )
        )
    ...
    def _record(self):
        self.test_record_Q.record(
            pos_curr=self.xQ,
            vel_curr=self.vQ,
            pos_d=self.xQd[self.timestep],
            vel_d=self.vQd[self.timestep],
        )
    ...
    def _get_reward(self):
        ...
        diffs = [
            norm(action_seq[i] - action_seq[i - 1], ord=2) for i in range(1, self.history_len + 1)
        ]
        eΔa = sum(exp_weights * diffs) / sum(exp_weights)

        rewards = exp(
            -np.array([scale_xQ, scale_vQ, scale_ψQ, scale_ωQ, scale_Δa])
    ...
    def _terminated(self):
        ...
        if exQ > self.pos_err_bound:
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Pos error: {round(exQ, 2)} | Time: {round(self.time_in_sec, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            return True
        elif evQ > self.vel_err_bound:
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Vel error: {round(evQ, 2)} | Time: {round(self.time_in_sec, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            return True
            ...
            self.num_episode += 1
            logger.info(
                f"Env {self.env_num} | Ep {self.num_episode} | Max time: {round(self.time_in_sec, 2)} | Final pos error: {round(exQ, 2)} | Reward: {round(self.total_reward, 1)}"
            )
            if self.is_record_action:
                self.plot_action()
            return True
