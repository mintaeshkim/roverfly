...
def local2global(data, model, local_point, body):
    ...
    global_point = np.array(data.xpos[body_id]).reshape(3, 1) + data.xmat[body_id].reshape(
        3, 3
    ) @ np.array(local_point).reshape(3, 1)
...
def draw_curve(viewer, points, width, rgba=[1, 0, 0, 1], body=None, model=None, data=None):
    ...
    if body is not None:
        if (model is None) or (data is None):
            ...
            raise ValueError(
                "Model and data must be provided to convert local points to global points."
            )
    ...
    **kwargs,
    ...
    if viewer is not None and torque_norm > 1e-8:
        draw_arrow(
            ...
            model=model,
            data=data,
            label=f"torque: {torque_norm:.2f} N-m",
        )
...
def get_cartesian_fluid_FT(m, d, body_ids):
    ...
    for body_id in body_ids:
        ...
        if body_id < 0 or body_id >= m.nbody:
            raise ValueError(
                f"Invalid body_id {body_id}, it must be in the range 0 to {m.nbody - 1}"
            )
