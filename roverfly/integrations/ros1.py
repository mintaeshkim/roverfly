...
from roverfly.math.rotations import quat2euler_raw, quat2rot
from roverfly.paths import MODEL_DIR
...
class RLOffboardNode:
    def __init__(self, model_path: str | Path | None = None):

        # Check Model
        default_model = MODEL_DIR / "payload_tvec_filter.onnx"
        model_path = Path(
            model_path or rospy.get_param("~model_path", str(default_model))
        ).expanduser()
        self.vehicle_name = rospy.get_param("~vehicle_name", "white_falcon")
        self.onnx_model = onnx.load(str(model_path))
        onnx.checker.check_model(self.onnx_model)
        self.ort_sess = ort.InferenceSession(str(model_path))
        print("RL_model_loaded")

        # subscriber to odometry
        self._quad_odom_sub = rospy.Subscriber(
            "/" + self.vehicle_name + "/odometry/mocap", Odometry, self._quad_odom_cb, queue_size=1
        )
        self._pyld_odom_sub = rospy.Subscriber(
            "/" + self.vehicle_name + "/payload/odometry/mocap",
            Odometry,
            self._pyld_odom_cb,
            queue_size=1,
        )
        ...
        self._pub_actions = rospy.Publisher(
            "/" + self.vehicle_name + "/actions", RLCommand, queue_size=1
        )
        ...
        # Variables
        self._home_position = np.array([0.0, 0.0, 0.0])
        self._home_takeoff_position = np.array([0.0, 0.0, 0.0])
        ...
        self.x_vel_Ctrl = PIDController(
            k_xy[0], k_xy[1], k_xy[2], self.policy_dt, 0, [-np.pi / 6, np.pi / 6]
        )  # limit pitch angle -np.pi/6,np.pi/6
        self.y_vel_Ctrl = PIDController(
            k_xy[0], k_xy[1], k_xy[2], self.policy_dt, 0, [-np.pi / 6, np.pi / 6]
        )  # limit roll angle
        self.z_vel_Ctrl = PIDController(
            k_z[0], k_z[1], k_z[2], self.policy_dt, 0, [-16, 8]
        )  # limit list thrust
        ...
        self.s_buffer = deque(
            np.zeros((self.history_len_long, self.s_len)), maxlen=self.history_len_long
        )
        ...
        self._dynreconfig_server = Server(
            RLOffboardManagerConfig, self._dynreconfig_server_callback
        )
    ...
    def request_takeoff(self):
        ...
        self._home_takeoff_position = self._home_position + np.array([0.0, 0.0, 1.5])
    ...
    def get_obs_curr(self):
        self.xQ = np.array(
            [
                self._quad_odom.pose.pose.position.x,
                self._quad_odom.pose.pose.position.y,
                self._quad_odom.pose.pose.position.z,
            ]
        )
        self.vQ = np.array(
            [
                self._quad_odom.twist.twist.linear.x,
                self._quad_odom.twist.twist.linear.y,
                self._quad_odom.twist.twist.linear.z,
            ]
        )
        ...
        self.q = np.array(
            [
                self._quad_odom.pose.pose.orientation.w,
                self._quad_odom.pose.pose.orientation.x,
                self._quad_odom.pose.pose.orientation.y,
                self._quad_odom.pose.pose.orientation.z,
            ]
        )
        self.ω = np.array(
            [
                self._quad_odom.twist.twist.angular.x,
                self._quad_odom.twist.twist.angular.y,
                self._quad_odom.twist.twist.angular.z,
            ]
        )
        ...
        while not rospy.is_shutdown():
            if self._enable_control:
                ...
                if not self._waiting_for_quadrotor_odom:  # and not self._waiting_for_payload_odom:
                    ...
                    if (
                        self._enable_RL
                    ):  # and self._falcon_state == self._falcon_states["air"]:  # Trajectory
                        ...
                        pitch_d = self.x_vel_Ctrl.compute(
                            self.vQ[0],
                            new_setpoint=np.clip(K_xy * (self.setpoint[0] - self.xQ[0]), -2, 2),
                        )  # Limit desired x velocity
                        roll_d = -self.y_vel_Ctrl.compute(
                            self.vQ[1],
                            new_setpoint=np.clip(K_xy * (self.setpoint[1] - self.xQ[1]), -2, 2),
                        )  # Limit desired y velocity
                        ...
                        ang_rate_d = (
                            np.array(
                                [
                                    np.clip(
                                        K_roll_pitch * (roll_d - euler[0]), -2 * np.pi, 2 * np.pi
                                    ),
                                    np.clip(
                                        K_roll_pitch * (pitch_d - euler[1]), -2 * np.pi, 2 * np.pi
                                    ),
                                    np.clip(K_yaw * (yaw_d - euler[2]), -2 * np.pi, 2 * np.pi),
                                ]
                            )
                            / np.pi
                        )
                        thrust_d = (
                            self.z_vel_Ctrl.compute(
                                self.vQ[2],
                                new_setpoint=np.clip(K_z * (self.setpoint[2] - self.xQ[2]), -5, 5),
                            )
                            / 8
                        )  # Limit desired z velocity
            ...
            if self._send:
                ...
                msg.thrust = (
                    10 * (action[0] + 1) / 2
                )  # Thrust (normalized) multiply mg to get thrust in N
                i = i + 0.001
                ...
                print(
                    f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(current_time))}.{milliseconds:03d}",
                    " action ",
                    i,
                )
...
if __name__ == "__main__":
    ...
    rospy.loginfo(f"Starting [{node_name}] node")

    RLOffboardNode()

    rospy.loginfo(f"Shutting down [{node_name}] node")
