...
import torch as th
import numpy as np
from envs.quadrotor_payload_multiple_env import QuadrotorPayloadEnv
from train.feature_extractor_multiple import CustomFeaturesExtractor
import argparse
import time
...
def main():
    ...
    model = PPO('MlpPolicy',  # CustomActorCriticPolicy,
                env=env,
                learning_rate=3e-4,
                n_steps=2048, # 2048  |  The number of steps to run for each environment per update / 2048 if dt=0.001 / 2048*16 if dt=0.01
                batch_size=512*num_cpu, # 512*num_cpu  |  *16 if dt=0.01
